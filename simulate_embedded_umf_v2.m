...
for i = 1:size(parameter, 1)
    [rmse(i, :), time(i)] = SIMULATE_UMF(parameter(i, :), config);
end
disp(time);
disp(rmse);
...
function [rmse, time] = SIMULATE_UMF(parameter, config)
...
for i = 1:test_count
    [image, x, y, r] = FUNC_EMBEDDED_SAMPLE_V2(ref_g, size_field, size_acquisition, size_block, size_pitch, 0, sigma, 1);
    [vec_m, vec_n, bias_m, bias_n, theta] = FUNC_EMBEDDED_ESTIMATE(image);
    [index_m, index_n, seq_p, indent_m, indent_n] = EXTRACT_UMF(image, size(image), size_window, size_pitch, bias_m, bias_n, vec_m, vec_n);
    try
        tic;
        [est_x, est_y, est_r] = LOCATE_UMF(ref_g, size_g, size_window, size_pitch, bias_m, bias_n, theta, index_m, index_n, seq_p, indent_m, indent_n);
        time(i) = toc;
        if (est_r - r > pi)
            est_r = est_r - 2 * pi;
        end
...
function [index_m, index_n, ext_p, indent_m, indent_n] = EXTRACT_UMF(image, size, window, pitch, bias_m, bias_n, vec_m, vec_n)
% gray_mean = mean(image, 'all');
gray_mean = 0.5;
coord_origin = [(size(1) + 1) / 2, (size(2) + 1) / 2] - bias_m .* vec_m - bias_n .* vec_n;
[index_m, index_n] = FIND_CODE_UMF(image, size, window, pitch, vec_m, vec_n, coord_origin, gray_mean);
...
for i = -(window(1)-1):window(1)-1
    for j = -(window(2)-1):window(2)-1
        [bound, value] = GET_CODE_UMF(image, size, vec_m, vec_n, coord_origin, gray_mean, index_m + pitch * i, index_n + pitch * j);
        if (bound)
            if (value)
...
function [index_m, index_n] = FIND_CODE_UMF(image, size, window, pitch, vec_m, vec_n, coord_origin, gray_mean)
% mathematically, for every n + 1 window there is a coded block.
index_m = 0;
index_n = 0;
for i = 0:pitch * window(1) % n + 1 points
    for j = 0:pitch * window(2)  % pitch points
        [bound, value] = GET_CODE_UMF(image, size, vec_m, vec_n, coord_origin, gray_mean, i, j);
        if (bound && value)
            index_m = mod(i, pitch);
            index_n = mod(j, pitch);
            return
...
function [bound, value] = GET_CODE_UMF(image, size, vec_m, vec_n, coord_origin, gray_mean, index_m, index_n)
bound = false;
value = false;
...
% Search function
function [index_x, index_y, rotation] = DECODE_UMF(ref_g, size_g, window, seq_p)
index_x = 0;
index_y = 0;
seq_p_90 = rot90(seq_p, -1);
seq_p_180 = rot90(seq_p_90, -1);
seq_p_270 = rot90(seq_p_180, -1);
...
for i = 1:size_g - window(1) + 1
    for j = 1:size_g - window(2) + 1
        ...
        if (all(ref_p == seq_p_90))
            index_x = i;
            index_y = j;
            rotation = 90;
            return
        end
        if (all(ref_p == seq_p_180))
            index_x = i;
            index_y = j;
            rotation = 180;
            return
        end
        if (all(ref_p == seq_p_270))
            index_x = i;
            index_y = j;
            rotation = 270;
            return
...
% Decode sequence
function [period_x, period_y, est_r] = LOCATE_UMF(ref_g, size_g, window, pitch, bias_m, bias_n, theta, index_m, index_n, seq_p, indent_m, indent_n)
[index_x, index_y, rotation] = DECODE_UMF(ref_g, size_g, window, seq_p);
period_x = 0;
period_y = 0;
if (rotation == 0)
